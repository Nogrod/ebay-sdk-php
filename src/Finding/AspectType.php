...
class AspectType implements \Sabre\Xml\XmlSerializable, \Sabre\Xml\XmlDeserializable
{
    ...
    {
        $value = self::mapArray($keyValue, '{http://www.ebay.com/marketplace/search/v1/services}valueHistogram', true);
        if (null !== $value && !empty($value)) {
            $this->setValueHistogram(array_map(function ($v) {
                return \Nogrod\eBaySDK\Finding\AspectValueHistogramType::fromKeyValue($v);
            }, $value));
        }
