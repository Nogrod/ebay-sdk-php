...
class ProductSearchPageType implements \Sabre\Xml\XmlSerializable, \Sabre\Xml\XmlDeserializable
{
    ...
    public function setKeyValue($keyValue)
    {
        ...
        if (null !== $value && !empty($value)) {
            $this->setSortCharacteristics(array_map(function ($v) {return \Nogrod\eBaySDK\Trading\CharacteristicType::fromKeyValue($v);}, $value));
        }
        $value = Func::mapArray($keyValue, '{urn:ebay:apis:eBLBaseComponents}DataElementSet', true);
        if (null !== $value && !empty($value)) {
            $this->setDataElementSet(array_map(function ($v) {return \Nogrod\eBaySDK\Trading\DataElementSetType::fromKeyValue($v);}, $value));
        }
    }
}
