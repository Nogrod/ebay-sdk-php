...
class RegulatoryType implements \Sabre\Xml\XmlSerializable, \Sabre\Xml\XmlDeserializable
{
    ...
    public function setKeyValue($keyValue)
    {
        ...
        if (null !== $value && !empty($value)) {
            $this->setResponsiblePersons(array_map(function ($v) {return \Nogrod\eBaySDK\Trading\ResponsiblePersonType::fromKeyValue($v);}, $value));
        }
        $value = Func::mapArray($keyValue, '{urn:ebay:apis:eBLBaseComponents}Documents', true);
        if (null !== $value && !empty($value)) {
            $this->setDocuments(array_map(function ($v) {return \Nogrod\eBaySDK\Trading\DocumentType::fromKeyValue($v);}, $value));
        }
    }
}
