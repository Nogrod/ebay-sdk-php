...
class ResponseAttributeSetType implements \Sabre\Xml\XmlSerializable, \Sabre\Xml\XmlDeserializable
{
    ...
    public function setKeyValue($keyValue)
    {
        ...
        if (null !== $value && !empty($value)) {
            $this->setProductFamilies(array_map(function ($v) {return \Nogrod\eBaySDK\Trading\ProductFamilyType::fromKeyValue($v);}, $value));
        }
        $value = Func::mapArray($keyValue, '{urn:ebay:apis:eBLBaseComponents}ProductFinderConstraints', true);
        if (null !== $value && !empty($value)) {
            $this->setProductFinderConstraints(array_map(function ($v) {return \Nogrod\eBaySDK\Trading\ProductFinderConstraintType::fromKeyValue($v);}, $value));
        }
