...
class ProductSuggestionsType implements \Sabre\Xml\XmlSerializable, \Sabre\Xml\XmlDeserializable
{
    ...
    public static function fromKeyValue($keyValue): \Nogrod\eBaySDK\Trading\ProductSuggestionsType
    {
        $self = new self();
    ...
    {
        $value = Func::mapArray($keyValue, '{urn:ebay:apis:eBLBaseComponents}ProductSuggestion');
        if (null !== $value) {
            $this->setProductSuggestion(array_map(function ($v) {return \Nogrod\eBaySDK\Trading\ProductSuggestionType::fromKeyValue($v);}, $value));
        }
    }
}
