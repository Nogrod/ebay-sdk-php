...
class OfferArrayType implements \Sabre\Xml\XmlSerializable, \Sabre\Xml\XmlDeserializable
{
     ...
     */
    public function addToOffer(\Nogrod\eBaySDK\MerchantData\OfferType $offer)
    {
        $this->offer[] = $offer;
        return $this;
    }
    ...
    {
        $value = self::mapArray($keyValue, '{urn:ebay:apis:eBLBaseComponents}Offer', true);
        if (null !== $value && !empty($value)) {
            $this->setOffer(array_map(function ($v) {
                return \Nogrod\eBaySDK\MerchantData\OfferType::fromKeyValue($v);
            }, $value));
        }
    }
