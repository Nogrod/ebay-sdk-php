...
class CharityAffiliationsType implements \Sabre\Xml\XmlSerializable, \Sabre\Xml\XmlDeserializable
{
    ...
    public function xmlSerialize(\Sabre\Xml\Writer $writer): void
    {
        ...
        if (null !== $value && !empty($this->getCharityID())) {
            $writer->write(array_map(function ($v) {return ["CharityID" => $v];}, $value));
        }
    }
    ...
    public function setKeyValue($keyValue)
    {
        ...
        if (null !== $value && !empty($value)) {
            $this->setCharityID(array_map(function ($v) {return \Nogrod\eBaySDK\MerchantData\CharityIDType::fromKeyValue($v);}, $value));
        }
    }
}
